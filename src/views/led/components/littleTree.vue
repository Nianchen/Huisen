  ...
  <div class="tree-list">
    <div class="group-left-img">
      ...
      <div class="tree-list-btn">
        ...
        <button class="button-active"><span>资源分类</span></button>
      </div>

      <!-- 分组 -->
      <!-- <div class="group-left-input" >
        <input type="text" v-model="filterText" placeholder="请输入资源名称" />
        <i class="el-icon-search"></i>
      </div>-->
      <div class="custom-tree-container" v-loading="isLoading" element-loading-text="拼命加载中" element-loading-spinner="el-icon-loading"
        element-loading-background="rgba(0, 0, 0, 0.4)">
        <el-tree id="el-tree" class="filter-tree" :data="groupTreeData" node-key="id" default-expand-all :filter-node-method="filterNode"
          :expand-on-click-node="false" ref="tree" @node-click="handleNodeClick" @node-contextmenu="rightClick">
          <span class="custom-tree-node" slot-scope="{ node, data }" @click="gotogrouplistInfo(data)" v-hasPermi="['led:component:view']">
      ...
      <div id="perTreeMenu" v-if="tmDisplay" class="tree_menu" :style="{...rightMenu}">
          <ul>
            ...
            <li @click.stop="() => remove(node, data)" v-hasPermi="['led:component:edit']"><i class="el-icon-delete"></i> 删除</li>
          </ul>
        </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    //获取分组数据
    getGroupTree() {
      resourceGroupdata().then((response) => {
    ...
    // 点击获取到当前分组id，传给兄弟组件
    gotogrouplistInfo(e) {
      this.$bus.$emit("littlegroupmessage", e.id);
      console.log(e.id);
    },
    ...
    handleBtnChange(flag) {
      console.log(flag);
      if (flag) {
        (this.showGroup = true), (this.showArea = false);
      } else {
        (this.showGroup = false), (this.showArea = true);
      }
      ...
      this.$prompt("请输入分组名称", "添加分组", {
        ...
        .then(({ value }) => {
          if (value) {
            ...
            addresourceGroup(this.form).then((response) => {
    ...
    remove(node, data) {
      ...
      this.$confirm("是否确认删除分组【" + data.label + "】?", "警告", {
        ...
        .then(() => {
          delresourceGroup(data.id).then((response) => {
      ...
      this.$prompt("请输入分组名称", "修改", {
        ...
        .then(({ value }) => {
          if (value) {
            ...
            updateresourceGroup(this.form).then((response) => {
