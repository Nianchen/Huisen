<template>
  <div class="tree-list">
    <div class="group-left-img">
      <div class="tree-list-btn">
        <button
          :class="isActive ? 'button-active' : ''"
          @click="handleBtnChange(true)"
        >
          <span>{{ btnTitle[0] }}</span>
        </button>
        <button
         v-show="!deptNone"
          :class="isActive ? '' : 'button-active'"
          @click="handleBtnChange(false)"
        >
          <span>{{ btnTitle[1] }}</span>
        </button>
      </div>

      <!-- 分组 -->
      <!-- <div class="group-left-input"  v-show="showGroup" >
        <input type="text" v-model="filterTextGroup" placeholder="请输入分组名称" />
        <i class="el-icon-search"></i>
      </div> -->
      <div  v-show="showGroup"
        class="custom-tree-container"
        v-loading="isLoading"
        element-loading-text="拼命加载中"
        element-loading-spinner="el-icon-loading"
        element-loading-background="rgba(0, 0, 0, 0.4)"
      >
        <el-tree
         v-show="showGroup"
          class="filter-tree"
          :data="groupTreeData"
          node-key="id"
          default-expand-all
          :filter-node-method="filterNode"
          :expand-on-click-node="false"
          ref="grouptree"
          @node-click="handleNodeClick"
          @node-contextmenu="rightClick"
        >
          <span class="custom-tree-node" slot-scope="{ node, data }">
            <span class="custom-tree-node-img" @click="gotogrouplistInfo(data)"  v-hasPermi="['broadcast:component:edit']">
              <img v-if="data && data.children" :src="node.expanded?folderOpenNewImg:folderNewImg"/>
              <img v-else :src="folderNewImg"  class="broadcastImg"/>
              <!-- <img  :src="images[0]"/> -->
              <span style="font-szie:30px">{{ data.label }}</span>
            </span>
            <!-- 三个按钮 -->
             <!-- <span>
              <el-button

                type="text"
                size="mini"
                @click.stop="() => append(data)"
                v-hasPermi="['broadcast:component:edit']"
              >
                <i class="el-icon-plus"></i>
              </el-button>
              <el-button
                class="buttons"
                type="text"
                size="mini"
                @click.stop="() => update(node, data)"
                v-hasPermi="['broadcast:component:edit']"
              >
                <i class="el-icon-edit"></i>
              </el-button>
              <el-button
                type="text"
                size="mini"
                @click.stop="() => remove(node, data)"
                v-hasPermi="['broadcast:component:edit']"
              >
                <i class="el-icon-delete"></i>
              </el-button>
            </span> -->
          </span>
        </el-tree>
      </div>

    <!-- 道路 -->

     <!-- <div class="group-left-input" v-show="showArea">
        <input type="text" v-model="filterTextArea" placeholder="请输入区域名称" />
        <i class="el-icon-search"></i>
      </div> -->
      <div    v-show="showArea"
        class="custom-tree-containerarea"
        v-loading="isLoading"
        element-loading-text="拼命加载中"
        element-loading-spinner="el-icon-loading"
        element-loading-background="rgba(0, 0, 0, 0.4)"
      >
        <el-tree
         v-show="showArea"
          class="filter-tree"
          :data="deptOptions"
          node-key="id"
          default-expand-all
          :filter-node-method="filterNode"
          :expand-on-click-node="false"
          ref="tree"
          @node-click="handleNodeClick"
        >
          <span class="custom-tree-node" slot-scope="{  node,data }">
            <span class="custom-tree-node-img" @click="gotoareaInfo(data)">
               <img v-if="data && data.children" :src="node.expanded?folderOpenImg:folderImg"/>
              <img v-else :src="folderImg"  style="font-size:20px"/>
              <span style="font-szie:30px">{{ data.label }}</span>
            </span>
          </span>
        </el-tree>
      </div>

    </div>
       <div id="perTreeMenu" v-if="tmDisplay" class="tree_menu" :style="{...rightMenu}">
          <ul>
            <li @click.stop="() => append(data)" v-hasPermi="['led:component:edit']"><i class="el-icon-tickets"></i> 新建</li>
            <li @click.stop="() => update(node, data)" v-hasPermi="['led:component:edit']"> <i class="el-icon-edit"></i> 修改</li>
            <li @click.stop="() => remove(node, data)" v-hasPermi="['led:component:edit']"><i class="el-icon-edit"></i> 删除</li>
          </ul>
        </div>
  </div>
</template>

<script>


import {
  groupTree,
  getGroupListData,
  addGroupList,
  updateGroupList,
  delGroupList,
  getGroupListInfo,
} from "@/api/broadcast/broadcastApi";
import { treeselect } from "@/api/system/dept";
import folderImg from "@/assets/images/camera/folder.png";
import broadcastimg from '@/assets/images/broadCastModel.png'

export default {
  name: "TreeList",
  props: {
    // title: {
    //   type: String,
    //   required: true,
    // },
    images: {
      type: Array,
      default: function () {
        return [folderImg,broadcastimg];
      },
    },
    btnTitle: {
      type: Array,
      default: function () {
        return ["分组", "道路"];
      },
    },

     deptNone:{
      require:false
    }
  },
  data() {
    return {
      tmDisplay: false,
      filterTextGroup:null,
      filterTextArea:null,
      isActive: true,
      groupTreeData: null,
      // 表单参数
      form: {},
      // 部门树选项
      deptOptions: null,
      isLoading:false,
      showGroup:true,
      showArea:false,
      folderOpenNewImg : require("@/assets/images/order-open.png"),
     folderNewImg : require("@/assets/images/order-close.png"),
       folderOpenImg : require("@/assets/images/road-close.png"),
     folderImg : require("@/assets/images/road-open.png"),
      data: null,
      node: null,
    };
  },
  watch: {
    filterTextGroup(val) {
      console.log(val);
      this.$refs.grouptree.filter(val);
    },
    filterTextArea(val) {
      this.$refs.tree.filter(val);
    },
  },
  created() {
    this.getGroupTree();
    this.getDeptTree();
  },
  methods: {
    // 右键点击函数
    rightClick(e, data, node) {
      console.log("e:", e, "data", data);
      this.rightMenu = { top: e.pageY+180+ "px", left: e.pageX+10 + "px" };
      this.tmDisplay = true;
      const self = this;
      document.onclick = function (ev) {
        if (ev.target !== document.getElementById("perTreeMenu")) {
          self.tmDisplay = false;
        }
      };
      this.data = data;
      this.node = node;
    },
    //获取分组详细数据


    gotogrouplistInfo(e) {

      if(this.deptNone){
        this.$bus.$emit('operationGroupId',e.id)
      }else{
         this.$bus.$emit('groupId',e.id)
      }

    },

    gotoareaInfo(e){
      // console.log(e);
      this.$bus.$emit('deptId',e.id)
    },

    /** 查询部门下拉树结构 */
    getDeptTree() {
      this.isLoading=true
      treeselect().then((response) => {
        this.deptOptions = response.data;
        this.isLoading=false
      });
    },
    //获取分组数据
    getGroupTree() {
         this.isLoading=true
      groupTree().then((response) => {
        this.groupTreeData = response.data;
        console.log(this.groupTreeData)
        this.$bus.$emit('broTreeData',this.groupTreeData)
        this.isLoading=false
      });
    },
    handleBtnChange(flag) {
         this.isActive = flag;
       if(flag){
        this.showGroup=true
        this.showArea=false
          this.filterTextGroup=null
          this.filterTextArea=null
      }else{
        this.showGroup=false
        this.showArea=true
          this.filterTextGroup=null
          this.filterTextArea=null
      }
      // this.handleTreeListBtnChange(flag)

    },

    handleNodeClick(data, node) {
      if (!data.children) {
        this.$emit("leafNodeChange", data);
      }
    },
    append(data) {
      this.reset();
      this.$prompt("请输入分组名称", "添加分组", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        inputValidator: (val) => {
          if (val === null) {
            return false;
          }
        },
        inputErrorMessage: "请输入分组名称",
      })
        .then(({ value }) => {
          console.log(data);
          if (value) {
            this.form.name = value;
            this.form.parentId = data.id;
            this.form.ancestors = data.ancestors + "," + data.id;
            addGroupList(this.form).then((response) => {
              this.msgSuccess("新增成功");
              this.getGroupTree();
            });
          }
        })
        .catch(() => {
          //   this.$message({
          //   showClose: true,
          //   message: '添加失败',
          //   type: 'error'
          // });
        });
    },
    remove(node, data) {
      if (node.childNodes.length !== 0) {
        this.$message.error("不能删除该分组，请先删除子分组");
        return;
      }
      this.$confirm("是否确认删除分组【" + data.label + "】?", "警告", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning",
      })
        .then(() => {
          delGroupList(data.id).then((response) => {
            this.msgSuccess("删除成功");
            this.getGroupTree();
          });
        })
        .catch(() => {
          this.$message({
            showClose: true,
            message: "删除失败",
            type: "error",
          });
        });
    },
    update(node, data) {
      this.$prompt("请输入分组名称", "修改", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        inputValue: data.label,
        inputValidator: (val) => {
          if (val === null) {
            return false;
          }
        },
        inputErrorMessage: "请输入区域名称",
      })
        .then(({ value }) => {
          console.log(value, data.id);
          if (value) {
            this.form.name = value;
            this.form.id = data.id;
            console.log(this.form);
            updateGroupList(this.form).then((response) => {
              this.msgSuccess("修改成功");
              this.getGroupTree();
            });
          }
        })
        .catch(() => {
          // this.$message({
          //   showClose: true,
          //   message: '修改失败',
          //   type: 'error'
          // });
        });
    },
    filterNode(value, data) {
      if (!value) return true;
      return data.label.indexOf(value) !== -1;
    },
    reset() {
      this.form = {
        id: null,
        name: null,
        code: null,
        parentId: null,
        ancestors: null,
        createBy: null,
        createTime: null,
        updateBy: null,
        updateTime: null,
        remarks: null,
        delFlag: null,
      };
    },
  },
};
</script>

<style lang="scss" scoped>
@import '@/assets/styles/poleIndex.scss';
.tree-list {
  width: 28.5vw;
  height: 87vh;
  padding-left: 10px;
  position: relative;

  .tree-list-btn {
    border-left: 3px solid rgba(4, 111, 148, 1);
    position: absolute;
    top: -4.1vh;
    left: 0.3vw;
    width: 80%;
    overflow: hidden;
    z-index: 1000;

    &::after {
      content: "";
      position: absolute;
      display: inline-block;
      height: 3.2vh;
      width: 1vw;
      background-image: radial-gradient(
        rgba(4, 111, 148, 0),
        rgba(4, 111, 148, 0.4),
        rgba(4, 111, 148, 1)
      );
      transform: skew(35deg);
      margin-left: 0.5vw;
      margin-top: 1.2vh;
      border: 2px solid rgba(4, 111, 148, 0.6);
    }

    button {
      background-color: transparent;
      background-image: radial-gradient(
        rgba(2, 94, 170, 0.1),
        rgba(2, 94, 170, 0.2),
        rgba(2, 94, 170, 0.4)
      );
      border: 3px solid rgba(4, 111, 148, 1);
      //border-bottom: none;
      transform: skew(35deg);
      outline: none;
      color: white;
      font-size: 1.2em;
      width: 6vw;
      height: 5vh;

      &:first-child {
        //border-right: none;
        width: 8vw;
        margin-left: -30px;
        padding-left: 30px;
      }

      &:last-child {
        margin-left: -4px;
      }

      &:active {
        background-image: radial-gradient(
          rgba(2, 94, 170, 0.1),
          rgba(2, 94, 170, 0.5),
          rgba(2, 90, 163, 0.9)
        );
      }

      span {
        display: block;
        transform: skew(-35deg);
      }
    }

    .button-active {
      background-image: radial-gradient(
        rgba(2, 94, 170, 0.1),
        rgba(2, 94, 170, 0.5),
        rgba(2, 90, 163, 0.9)
      );
    }
  }

  .group-left-img {
     position: relative;
    margin-top: 0.4vh;
    width: 71%;
    height: 86%;
    top: 2vh;
    transform: translateY(3%);
    background: no-repeat url("../../../assets/images/tree-bac.png");
    background-size: 100% 98%;
    .title {
      position: absolute;
      top: 5%;
      left: 22%;
      font-size: 1.5em;
      color: rgba(255, 255, 255, 0.5);
    }

    .group-left-input {
      position: absolute;
      top: 5%;
      right: 10%;
      width: 55%;
      height: 5%;
      display: flex;
      align-items: center;
      border: 2px solid rgba(14, 93, 160, 0.5);
      border-radius: 30px;
      padding-right: 0.8vw;

      input {
        width: 95%;
        height: 100%;
        background: none;
        outline: none;
        border: none;
        padding: 0 0.8vw;
        color: white;
        font-size: 1em;
      }

      input::-webkit-input-placeholder {
        /* Chrome/Opera/Safari */
        color: rgba(255, 255, 255, 0.3);
      }

      input::-moz-placeholder {
        /* Firefox 19+ */
        color: rgba(255, 255, 255, 0.3);
      }

      input:-ms-input-placeholder {
        /* IE 10+ */
        color: rgba(255, 255, 255, 0.3);
      }

      input:-moz-placeholder {
        /* Firefox 18- */
        color: rgba(255, 255, 255, 0.3);
      }
    }
  }
}

.custom-tree-container {
  //border: 1px solid;
  position: absolute;
  top: 46%;
  left: 51%;
  transform: translate(-50%, -50%);
  width: 16vw;
  height: 63vh;
  //max-height: 28vh;
  overflow: auto;

  //max-width: 300px;
}

.custom-tree-containerarea {
  //border: 1px solid;
 position: absolute;
  top: 49%;
  left: 50%;
  transform: translate(-50%, -50%);
  width: 19vw;
  height: 63vh;
  //max-height: 28vh;
  // overflow: auto;

  //max-width: 300px;
}

/*.custom-tree-container::-webkit-scrollbar {
  display: none; !* Chrome Safari *!
}

.custom-tree-container {
  scrollbar-width: none; !* firefox *!
  -ms-overflow-style: none; !* IE 10+ *!
  overflow-x: hidden;
  overflow-y: auto;
}*/

::-webkit-scrollbar {
  width: 5px;
  height: 5px;
}

::-webkit-scrollbar-thumb {
  border-radius: 1em;
  background-color: rgba(56, 104, 153, 0.3);
}

::-webkit-scrollbar-track {
  border-radius: 1em;
  background-color: rgba(56, 104, 153, 0.1);
}

.custom-tree-node {
  flex: 2;
  display: flex;
  align-items: center;
  justify-content: space-between;
  font-size: 1.1em;
  padding-right: 8px;

  .custom-tree-node-img {

    display: flex;
    justify-content: center;
    align-items: center;
  img{
      width: 1.3vw;
    }

  }

  .buttons {
    padding-left: 5px;
  }
}

.el-tree {
  background-color: transparent;

  .el-button {
    color: #fff;
    font-size: 1em;
  }

  .el-icon-s-order {
    font-size: 17px;
  }

  .custom-tree-node {
    color: #fff;
  }
}

</style>
